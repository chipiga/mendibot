      ...
      def process(command, arg)
        ...
        when 'status' # Get user status and try translate it to english
          begin
            ...
            from = Plugins::Translate.translator.detect_language(result)['language']
            to = 'en'
            result = Plugins::Utils.entities.decode(Plugins::Translate.translator.translate(from.to_sym, to.to_sym, result)) unless from == to
            # result = Plugins::Translate.process('en', ::Twitter.user(arg).status.text)
          rescue Exception => e
        ...
        when 'update' # Update user status, automatically short URLs through bit.ly
          begin
            URI.extract(arg, "http").each{|url| arg.sub!(url, Plugins::Bitly.process('shorten', url))}
